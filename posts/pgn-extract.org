...
[fn:2] Since writing this post, I’ve learned that although this mating pattern can be achieved with a queen, it is technically only called Anastasia’s Mate when the checkmate is delivered by a rook. However, I’ve left the article unchanged, as it still demonstrates a useful feature of pgn-extract.
